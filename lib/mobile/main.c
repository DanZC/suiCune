...
// asm_1103d6:
    ...
    // LD_A(0xc8);
    // LD_hl_A;
    gMobile_wc829_wc82a = (uint8_t*)wram->wc880_str;
...
uint8_t Function110596(mobile_api_data_s* data, uint8_t b){
    ...
    // LD_A(0xc8);
    // LD_hl_A;
    gMobile_wc829_wc82a = &wram->wc880;
    // CALL(aFunction1104b0);
    Function1104b0();
    // LD_A_addr(wcb36);
    // LD_addr_A(wc86a);
...
void Function110af4(mobile_api_data_s* data){
    ...
    if(data->bc != 0) {
        ...
        // asm_110bd5:
            ...
            if(data->bc <= e) {
                ...
                // LD_D_A;
                // CALL(aMobileSDK_CopyBytes);
                data->de = MobileSDK_CopyBytes(gMobile_wc829_wc82a, wram->wc880_str + (0x80 - e), LOW(data->bc));
                // LD_HL(wc827);
                // LD_A_hli;
                // LD_H_hl;
                // LD_L_A;
        ...
        // asm_110c05:
            ...
            // LD_A_D;
            // LD_addr_A(wc82a);
            gMobile_wc829_wc82a = MobileSDK_CopyBytes(gMobile_wc829_wc82a, wram->wc880_str + (0x80 - e), wram->wc993);
            // POP_BC;
            // LD_A_addr(wc993);
            // LD_E_A;
            // LD_A_C;
    ...
    // asm_110b1c:
        ...
        if(data->bc <= e) {
            ...
            // LD_D_A;
            // CALL(aMobileSDK_CopyBytes);
            data->de = MobileSDK_CopyBytes(gMobile_wc829_wc82a, wram->wMobileSDK_ReceivePacketBuffer + 5 + (wram->wMobileSDK_ReceivePacketBuffer[3] - e - 1), c);
            // POP_BC;
            // LD_HL(wc827);
            // LD_A_hli;
            // LD_H_hl;
...
// asm_110b5c:
    ...
    if(wram->wc86e != 0) {
        ...
        // LD_D_A;
        // CALL(aMobileSDK_CopyBytes);
        data->de = MobileSDK_CopyBytes(gMobile_wc829_wc82a, wram->wMobileSDK_ReceivePacketBuffer + 5 + (wram->wMobileSDK_ReceivePacketBuffer[3] - e - 1), e);
        // LD_HL(wc829);
        // LD_A_E;
        // LD_hli_A;
        // LD_A_D;
    ...
    if(b == (MOBILE_COMMAND_ERROR | 0x80)) {
        ...
        switch(wram->wMobileSDK_SendCommandID) {
            ...
            // asm_111bf0:
                ...
                // INC_HL;
                // CALL(aMobileSDK_CopyBytes);
                MobileSDK_CopyBytes(gMobile_wc829_wc82a, wram->wMobileSDK_ReceivePacketBuffer + 5, wram->wMobileSDK_ReceivePacketBuffer[3] - 1);
                // LD_A(0x2);
                // LD_addr_A(wc807);
                wram->wc807 = 0x2;
                // RET;
...
void Function112430(void){
    ...
    *de = wram->wc86c;
    ...
    // LD_HL(wMobileSDK_PacketBuffer);
    // JP(mFunction111f02);
    Function111f02(wram->wMobileSDK_PacketBuffer, size, 0xa4);
}
...
const char CrystalManufactureCode[] = { 'B', 'Y', 'T', 'E' };
...
// MobileSDK_CopyPOSTHeaderInternal?
...
void Function112d33(uint8_t* hl, uint8_t a){
    ...
    // asm_112dc1:
        ...
        asm_112e46:
            ...
            asm_112e95:
                // LD_addr_A(wc80f);
                wram->wc80f = (wram->wc990 == 0x1)? 0x32: 0x33;
                // LD_HL(wc810);
                // LD_A_de;
                // INC_DE;
                // LD_hli_A;
        ...
        // asm_112ec1:
            ...
            // OR_A_L;
            // IF_NZ goto asm_112efb;
            if(wram->wc9a5[0] != 0x0 || (wram->wc86e | wram->wc86f) != 0)
                goto asm_112efb;
            // LD_A(0x2);
            // LD_addr_A(wc86a);
            wram->wc86a = 0x2;
...
uint8_t Function112f61(void){
    ...
    if(wram->wc989 == 0) {
        ...
        // LD_DE(0x0008);
        // ADD_HL_DE;
        const char* hl = (const char*)wram->wc884 + 0x4;
...
// asm_112f8a:
    ...
    // asm_112fa1:
        ...
        hl = Function112fd5(hl);
        // CALL(aFunction113008);
        hl = Function113008(hl);
        // CALL(aFunction113026);
        hl = Function113026(hl);
        // CALL(aFunction113054);
        hl = Function113054(hl);
        // CALL(aFunction113167);
        hl = Function113167(hl);
        // CALL(aFunction113180);
        hl = Function113180(hl);
...
void Function11306b(void){
    ...
    return Function113072(wram->wc880_str, wram->wc82d);
}
    ...
    while(1) {
        ...
        if(res.flag) { // Inlined
            ...
            if(*hl != '\r') {
                ...
                if(*hl == '\n')
                    return Function113095(hl, res.a);
                // JR(mFunction113072);
                continue;
            }
...
// asm_11310d:
    ...
    // OR_A_A;
    // IF_Z goto asm_113150;
    if(wram->wMobileSDK_ReceivePacketBuffer[0] != 0x9f && wram->wMobileSDK_ReceivePacketBuffer[3] != 0) {
        // LD_L_C;
        uint8_t l = c;
        // SUB_A_C;
        // LD_C_A;
        ...
        // LD_HL(wMobileSDK_ReceivePacketBuffer + 4);
        // ADD_HL_BC;
        uint8_t* hl2 = wram->wMobileSDK_ReceivePacketBuffer + 4 + l;
        // LD_B_A;
        b = l;
        // PUSH_DE;
        // LD_A_addr(wc82b);
...
void Function113317(void){
    ...
    wram->wc86b = 0x1;
    // LD_DE(wMobileSDK_PacketBuffer + 18);
    char* de = (char*)wram->wMobileSDK_PacketBuffer;
    // LD_A_addr(wc86c);
    // LD_de_A;
    // INC_DE;
    *(de++) = wram->wc86c;
...
asm_113351:
    ...
    // LD_HL(wMobileSDK_PacketBuffer + 12);
    // JP(mFunction111f02);
    Function111f02(wram->wMobileSDK_PacketBuffer + 12, bc, 0x95);
}
...
void Function113519(uint8_t* hl, uint8_t a){
    ...
    // asm_11352d:
        ...
        // LD_A(0xc8);
        // LD_hli_A;
        gMobile_wc829_wc82a = wram->wc8ca + 0x16;
        // LD_HL(mMobilePacket_ReadConfigurationDataPart2);
        // JP(mFunction11236b);
        Function11236b(MobilePacket_ReadConfigurationDataPart2);
        return;
...
char* Function1135ba(char* de){
    ...
    de = Function1135eb(de, (const char*)wram->wc8f6);
    uint16_t bc = 0;
    // LD_A(0x11);
    // LD_HL(wc8fe);
    // CALL(aMobileSDK_CopyStringLen);
    ...
    // LD_HL(wc90e);
    // CALL(aFunction1135eb);
    de = Function1135eb(de, (const char*)wram->wc90e);
    // LD_A(0x11);
    // LD_HL(wc916);
    // CALL(aMobileSDK_CopyStringLen);
    ...
    // LD_HL(wc926);
    // CALL(aFunction1135eb);
    de = Function1135eb(de, (const char*)wram->wc926);
    printf("Copying \"%s\"\n", de2);
    // LD_A(0x11);
    // LD_HL(wc92e);
    // JP(mMobileSDK_CopyStringLen);
...
char* Function1135eb(char* de, const char* hl){
    // LD_B(0x8);
    uint8_t b = 0x8;
    ...
    // asm_1135ed:
        ...
        if(c == 0x3a)
            c = (char)Function113620();
        // CP_A(0x3b);
        // CALL_Z (aFunction113623);
        if(c == 0x3b)
            c = (char)Function113623();
        // LD_de_A;
        // INC_DE;
        *(de++) = c;
        // LD_A_hli;
