...
void TrainerCard(void){
    ...
    // loop:
        ...
        // AND_A(B_BUTTON);
        // IF_NZ goto quit;
        if(bit_test(wram->wJumptableIndex, 7) || bit_test(hJoyLast, B_BUTTON))
            break;
...
void TrainerCard_PrintTopHalfOfCard(void){
    ...
    PlaceGraphicYStagger_Conv(coord(14, 1, wram->wTilemap), 5, 7);
    // RET;
}
