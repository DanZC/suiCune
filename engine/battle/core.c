...
// alive:
    ...
    // IF_Z goto player_2;
    if(wram->wLinkMode == LINK_NULL || hSerialConnectionStatus != USING_INTERNAL_CLOCK) {
    // not_linked:
...
bool ResidualDamage(void){
    ...
    // got_anim:
        ...
        uint8_t* toxicCount = (hBattleTurn == 0)? &wram->wPlayerToxicCount: &wram->wEnemyToxicCount;
...
// not_cursed:
    ...
    // LD_HL(wEnemyMonHP);
    uint16_t hp = (hBattleTurn == 0)? wram->wBattleMon.hp: wram->wEnemyMon.hp;
...
static void HandlePerishSong_do_it(void){
    ...
    uint8_t* hl = (hBattleTurn == 0)? &wram->wPlayerPerishCount: &wram->wEnemyPerishCount;
...
static void HandleWrap_do_it(void){
    ...
    uint8_t* wrapCount = (hBattleTurn == 0)? &wram->wPlayerWrapCount: &wram->wEnemyWrapCount;
    move_t* trappingMove = (hBattleTurn == 0)? &wram->wPlayerTrappingMove: &wram->wEnemyTrappingMove;
    ...
    // loop:
        ...
        // player_pp:
            ...
            if(moves[c] == moves2[c]){
                ...
                uint8_t ss5 = (hBattleTurn == 0)? wram->wPlayerSubStatus5: wram->wEnemySubStatus5;
        ...
        // skip_checks:
            // CALLFAR(aGetUserItem);
            item_t* userItem = (hBattleTurn == 0)? &wram->wBattleMon.item: &wram->wEnemyMon.item;
            // LD_A_hl;
            // LD_addr_A(wNamedObjectIndex);
            ...
            *userItem = NO_ITEM;
            ...
            item_t* partyItem = (hBattleTurn == 0)? &wram->wPartyMon[wram->wCurBattleMon].mon.item: &wram->wOTPartyMon[wram->wCurOTMon].mon.item;
            // LDH_A_addr(hBattleTurn);
            // AND_A_A;
...
static void HandleWeather_SandstormDamage(void) {
    ...
    uint8_t* types = (hBattleTurn == 0)? wram->wBattleMon.types: wram->wEnemyMon.types;
...
void SubtractHP_Conv(uint16_t bc){
    ...
    // IF_Z goto ok;
    // LD_HL(wEnemyMonHP);
    uint16_t* hp = (uint16_t*)((hBattleTurn == 0)? wram_ptr(wBattleMonHP): wram_ptr(wEnemyMonHP));
...
//  output: bc, wHPBuffer1
    ...
    // IF_Z goto ok;
    // LD_HL(wEnemyMonMaxHP);
    uint16_t hp = ReverseEndian16((hBattleTurn == 0)? wram->wBattleMon.maxHP: wram->wEnemyMon.maxHP);
...
void HandleHPHealingItem(void){
    ...
    // LD_HL(wBattleMonMaxHP);
    uint8_t* hp_    = (uint8_t*)((hBattleTurn == 0)? &wram->wEnemyMon.hp    : &wram->wBattleMon.hp);
    uint8_t* maxhp_ = (uint8_t*)((hBattleTurn == 0)? &wram->wEnemyMon.maxHP : &wram->wBattleMon.maxHP);
...
// okay:
    ...
    // hlcoord(10, 9, wTilemap);
    uint8_t* hl = (hBattleTurn == 0)?  coord(2, 2, wram->wTilemap): coord(10, 9, wram->wTilemap);

// got_hp_bar_coords:
    // LD_addr_A(wWhichHPBar);
    // PREDEF(pAnimateHPBar);
    AnimateHPBar_Conv(hl, hBattleTurn);
