...
static void v_SwitchPartyMons_ClearSprite(uint8_t a) {
    ...
    // LD_A(0x7f);
    // CALL(aByteFill);
    ByteFill_Conv2(coord(0, 1, wram->wTilemap) + 2 * SCREEN_WIDTH * a, 2 * SCREEN_WIDTH, 0x7f);
    // POP_AF;
...
static void v_SwitchPartyMons_SwapMonAndMail(uint8_t from, uint8_t to) {
    ...
    *species_to = temp_species;
    ...
    // LD_BC(PARTYMON_STRUCT_LENGTH);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(wram->wSwitchMonBuffer, wram->wPartyMon + to, PARTYMON_STRUCT_LENGTH);
    // LD_A_addr(wSwitchMonFrom);
    // LD_HL(wPartyMon1);
    // LD_BC(PARTYMON_STRUCT_LENGTH);
    // CALL(aAddNTimes);
    ...
    // LD_BC(PARTYMON_STRUCT_LENGTH);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(wram->wPartyMon + to, wram->wPartyMon + from, PARTYMON_STRUCT_LENGTH);
    // POP_DE;
    // LD_HL(wSwitchMonBuffer);
    // LD_BC(PARTYMON_STRUCT_LENGTH);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(wram->wPartyMon + from, wram->wSwitchMonBuffer, PARTYMON_STRUCT_LENGTH);
    // LD_A_addr(wSwitchMonTo);
    // LD_HL(wPartyMonOTs);
    // CALL(aSkipNames);
    // PUSH_HL;
    // CALL(av_SwitchPartyMons_CopyNameToSwitchMonBuffer);
    CopyBytes_Conv2(wram->wSwitchMonBuffer, wram->wPartyMonOT[to], NAME_LENGTH);
    // LD_A_addr(wSwitchMonFrom);
    // LD_HL(wPartyMonOTs);
    // CALL(aSkipNames);
    // POP_DE;
    ...
    CopyBytes_Conv2(wram->wPartyMonOT[to], wram->wPartyMonOT[from], NAME_LENGTH);
    // POP_DE;
    // LD_HL(wSwitchMonBuffer);
    // CALL(av_SwitchPartyMons_CopyName);
    CopyBytes_Conv2(wram->wPartyMonOT[from], wram->wSwitchMonBuffer, NAME_LENGTH);
    // LD_HL(wPartyMonNicknames);
    // LD_A_addr(wSwitchMonTo);
    // CALL(aSkipNames);
    // PUSH_HL;
    // CALL(av_SwitchPartyMons_CopyNameToSwitchMonBuffer);
    CopyBytes_Conv2(wram->wSwitchMonBuffer, wram->wPartyMonNickname[to], NAME_LENGTH);
    // LD_HL(wPartyMonNicknames);
    // LD_A_addr(wSwitchMonFrom);
    // CALL(aSkipNames);
    // POP_DE;
    // PUSH_HL;
    // CALL(av_SwitchPartyMons_CopyName);
    CopyBytes_Conv2(wram->wPartyMonNickname[to], wram->wPartyMonNickname[from], NAME_LENGTH);
    // POP_DE;
    // LD_HL(wSwitchMonBuffer);
    // CALL(av_SwitchPartyMons_CopyName);
    CopyBytes_Conv2(wram->wPartyMonNickname[from], wram->wSwitchMonBuffer, NAME_LENGTH);
    // LD_HL(sPartyMail);
    // LD_A_addr(wSwitchMonTo);
    ...
    // CALL(aOpenSRAM);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(wram->wSwitchMonBuffer, hl + to, MAIL_STRUCT_LENGTH);
    // LD_HL(sPartyMail);
    // LD_A_addr(wSwitchMonFrom);
    // LD_BC(MAIL_STRUCT_LENGTH);
    // CALL(aAddNTimes);
    ...
    // LD_BC(MAIL_STRUCT_LENGTH);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(hl + to, hl + from, MAIL_STRUCT_LENGTH);
    // POP_DE;
    // LD_HL(wSwitchMonBuffer);
    // LD_BC(MAIL_STRUCT_LENGTH);
    // CALL(aCopyBytes);
    CopyBytes_Conv2(hl + from, wram->wSwitchMonBuffer, MAIL_STRUCT_LENGTH);
    // CALL(aCloseSRAM);
    CloseSRAM_Conv();
    // POP_BC;
    // POP_DE;
