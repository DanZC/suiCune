...
// useless_pointer:
    ...
    if(!MartAskPurchaseQuantity() && MartConfirmPurchase()){
        ...
        u8_flag_s res = CompareMoney_Conv(hMoneyTemp, wram->wMoney);
        if(res.flag) {
        // insufficient_funds:
        ...
        // LD_BC(hMoneyTemp);
        // CALL(aTakeMoney);
        TakeMoney_Conv(wram->wMoney, hMoneyTemp);
...
bool BargainShopAskPurchaseQuantity(void){
    ...
    hMoneyTemp[2] = price & 0xff;
    // LD_A_hl;
    // LDH_addr_A(hMoneyTemp + 1);
    hMoneyTemp[1] = (price >> 8) & 0xff;
    // XOR_A_A;
        ...
        // okay_to_sell:
            ...
            if(!declined) {
                ...
                if(YesNoBox_Conv()) {
                    ...
                    // LD_BC(hMoneyTemp);
                    // CALL(aGiveMoney);
                    GiveMoney_Conv(wram->wMoney, hMoneyTemp);
                    // LD_A_addr(wMartItemID);
                    // LD_HL(wNumItems);
                    // CALL(aTossItem);
