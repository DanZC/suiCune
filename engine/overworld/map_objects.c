...
void MovementFunction_RandomWalkY(struct Object* bc) {
    ...
    v_RandomWalkContinue(bc, hRandomAdd & 0b00000001);
}
...
void MovementFunction_RandomWalkX(struct Object* bc) {
    ...
    // OR_A(0b00000010);
    // JP(mv_RandomWalkContinue);
    v_RandomWalkContinue(bc, (hRandomAdd & 0b00000001) | 0b00000010);
}
...
void MovementFunction_RandomWalkXY(struct Object* bc) {
    ...
    // AND_A(0b00000011);
    // JP(mv_RandomWalkContinue);
    v_RandomWalkContinue(bc, hRandomAdd & 0b00000011);
}
...
void MovementFunction_RandomSpinSlow(struct Object* bc) {
    ...
    // ADD_HL_BC;
    // LD_hl_A;
    bc->facing = hRandomAdd & 0b00001100;
...
void MovementFunction_RandomSpinFast(struct Object* bc) {
    ...
    // LDH_A_addr(hRandomAdd);
    // AND_A(0b00001100);
    uint8_t a = hRandomAdd & 0b00001100;
    // CP_A_D;
    // IF_NZ goto keep;
...
void RandomStepDuration_Slow(struct Object* bc) {
    ...
    return v_SetRandomStepDuration(bc, hRandomAdd & 0b01111111);
}
...
void RandomStepDuration_Fast(struct Object* bc) {
    ...
    // LDH_A_addr(hRandomAdd);
    // AND_A(0b00011111);
    return v_SetRandomStepDuration(bc, hRandomAdd & 0b00011111);
}
...
void CopyTempObjectData_Conv(struct Object* bc, const uint8_t* de) {
    ...
    // LDH_A_addr(hMapObjectIndex);
    // LD_hli_A;
    wram->wTempObjectCopyRange = hMapObjectIndex;
    // PUSH_HL;
    // LD_HL(OBJECT_NEXT_MAP_X);
    // ADD_HL_BC;
    // LD_D_hl;
...
void RespawnPlayerAndOpponent_Conv(void) {
    ...
    // AND_A_A;
    // IF_Z goto skip_opponent;
    if(bit_test(wram->wBattleScriptFlags, 7) && hLastTalked != 0) {
        // CALL(aRespawnObject);
        RespawnObject_Conv(hLastTalked);
    }
...
void HideAllObjects(void) {
    ...
    // loop:
        ...
        // CP_A(NUM_OBJECT_STRUCTS);
        // IF_NZ goto loop;
    } while(++hMapObjectIndex != NUM_OBJECT_STRUCTS);
    // RET;
}
    ...
    if(x >= 0xf0 || x < SCREEN_WIDTH_PX) {
        ...
        // loop:
            do {
                ...
                // ADD_A_E;
                // DEC_A;
                uint8_t ycoord = (hCurSpriteYCoord + e) - 1;
                // CP_A(SCREEN_HEIGHT);
                // IF_NC goto ok9;
                ...
                // next:
                    ...
                    // ADD_A_D;
                    // DEC_A;
                    uint8_t xcoord = (hCurSpriteXCoord + d) - 1;
                    // CP_A(SCREEN_WIDTH);
                    // IF_NC goto ok8;
...
bool SetLeaderIfVisible_Conv(uint8_t a) {
    ...
    // LDH_A_addr(hObjectStructIndex);
    // LD_addr_A(wObjectFollow_Leader);
    wram->wObjectFollow_Leader = hObjectStructIndex;
    // RET;
    return true;
}
...
bool SetFollowerIfVisible_Conv(uint8_t a) {
    ...
    // LDH_A_addr(hObjectStructIndex);
    // LD_addr_A(wObjectFollow_Follower);
    wram->wObjectFollow_Follower = hObjectStructIndex;
    // RET;
    return true;
}
...
// ok:
    ...
    // CP_A_B;
    // RET_NC;
    uint8_t sprite_idx = hUsedSpriteIndex >> 2;
    if(sprite_idx >= b) {
        hOAMUpdate = temp;
        return;
    }
...
// not_overhead:
    ...
    hCurSpriteOAMFlags = (bit_test(flags2, OVERHEAD_F))? PRIORITY: 0;
    // LD_HL(OBJECT_SPRITE_X);
    // ADD_HL_BC;
    // LD_A_hl;
    // LD_HL(OBJECT_SPRITE_X_OFFSET);
    ...
    // ADD_A_E;
    // LDH_addr_A(hCurSpriteXPixel);
    hCurSpriteXPixel = bc->spriteX + bc->spriteXOffset + 8 + wram->wPlayerBGMapOffsetX;
    ...
    hCurSpriteYPixel = bc->spriteY + bc->spriteYOffset + 12 + wram->wPlayerBGMapOffsetY;
    // LD_HL(OBJECT_FACING_STEP);
    // ADD_HL_BC;
    // LD_A_hl;
    ...
    if(step != (uint8_t)STANDING && step < NUM_FACINGS) {
        ...
        struct SpriteOAM* bc = wram->wVirtualOAMSprite + (hUsedSpriteIndex >> 2);
        // LD_A_hli;
        a = hl->count;
    ...
    // addsprite:
        ...
        do {
            ...
            // LD_bc_A;  // y
            // INC_C;
            bc[idx].yCoord = hCurSpriteYPixel + hl->data[idx].y;
            // LDH_A_addr(hCurSpriteXPixel);
            // ADD_A_hl;
            // INC_HL;
            // LD_bc_A;  // x
            // INC_C;
            bc[idx].xCoord = hCurSpriteXPixel + hl->data[idx].x;
            // LD_E_hl;
            // INC_HL;
            ...
            // IF_Z goto nope1;
            // XOR_A_A;
            a = (!bit_test(attr, ABSOLUTE_TILE_ID_F))? hCurSpriteTile: 0;
        ...
        // nope1:
            ...
            // OR_A_E;
            a = (bit_test(attr, RELATIVE_ATTRIBUTES_F))? hCurSpriteOAMFlags | attr: attr;
        ...
        // nope2:
            ...
            // LD_A_C;
            // LDH_addr_A(hUsedSpriteIndex);
            hUsedSpriteIndex += sizeof(struct SpriteOAM) * idx;
            break;
        } while(1);
    }
